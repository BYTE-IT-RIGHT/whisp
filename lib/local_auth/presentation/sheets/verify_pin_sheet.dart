...
class VerifyPinSheet extends StatefulWidget {
  final WhispTheme theme;
  final LocalAuthCubit localAuthCubit;

  const VerifyPinSheet({
    super.key,
    required this.theme,
  ...
  static Future<void> show({
    ...
    return showModalBottomSheet(
      ...
      isDismissible: true,
      enableDrag: true,
      builder: (sheetContext) => VerifyPinSheet(
        theme: theme,
  ...
  @override
  State<VerifyPinSheet> createState() => _VerifyPinSheetState();
}

class _VerifyPinSheetState extends State<VerifyPinSheet> {
  String _pin = '';
  ...
  Future<void> _handlePinComplete(String pin) async {
    ...
    await widget.localAuthCubit.authenticateWithPin(pin);

    // If still mounted and still in loading state (not authenticated),
    // the PIN was incorrect
    if (mounted) {
      final state = widget.localAuthCubit.state;
